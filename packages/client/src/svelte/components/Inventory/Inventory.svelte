<script lang="ts">
  ...
  $: {
    inventoryItems = Object.entries($entities).filter(
      ([itemId, item]) => item.carriedBy === baseEntityId
    )
  ...
  const onDrop = (event: any) => {
    ...
    addToSequencer(WorldFunctions.Transfer, [item, baseEntityId])
...
{#if showCapacity}
  <div class="cc">
    Inventory: {$itemCount}/{$baseEntities[baseEntityId]
      ? $baseEntities[baseEntityId].carryingCapacity
      : 0}
  </div>
{/if}
...
<div class="inventory" class:draggingOver>
  ...
  {#if showEmptySlots}
    {#each Array(emptySlotNumber) as _, i (i)}
      <EmptySlot
        dropAllowed={!isPlayer &&
          !inventoryItems.map(i => i[0]).includes($dragging)}
        on:drop={onDrop}
      />
    {/each}
  {/if}
